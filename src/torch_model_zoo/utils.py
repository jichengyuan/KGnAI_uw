...
def anno_filter(anno_path, filter_cat_nms):
    """
    Filters a COCO annotation file to include only the specified categories,
    ensuring the final category order matches the 'filter_cat_nms' list.

    Args:
        anno_path (str): Path to the COCO annotation file (e.g., 'annotations.json').
        filter_cat_nms (list): A list of category names to keep, in the desired order.
                               Example: ['person', 'car', 'bus']

    Returns:
        dict: A COCO-formatted dictionary containing the filtered and re-ordered data.
    """
    coco = COCO(anno_path)
    
    # --- THIS IS THE CORRECTED LOGIC ---
    # 1. Build the category list and ID mappings by iterating through 
    #    filter_cat_nms to enforce the desired order.
    
    categoreis_info = []
    # This will map the ORIGINAL category ID to our NEW, sequential category ID (starting from 1)
    original_to_new_cat_id_map = {}
    
    # Keep track of only the names that actually exist in the annotation file
    valid_filter_cat_nms = []

    for i, cat_name in enumerate(filter_cat_nms):
        # Get the original category ID for the current name
        original_cat_id = coco.getCatIds(catNms=[cat_name])
        
        # If the category exists in the annotation file...
        if original_cat_id:
            new_cat_id = i + 1 # New, sequential ID (1, 2, 3, ...)
            valid_filter_cat_nms.append(cat_name) # Add to our list of valid names
            
            # Load the original category info
            original_cat_info = coco.loadCats(original_cat_id)[0]
            
            # Update its ID to our new sequential ID
            original_cat_info['id'] = new_cat_id
            
            # Add it to our correctly ordered list of categories
            categoreis_info.append(original_cat_info)
            
            # Store the mapping from old ID to new ID
            original_to_new_cat_id_map[original_cat_id[0]] = new_cat_id
    ...
    # 2. Get all images that contain any of the valid categories
    cat_ids_to_keep = list(original_to_new_cat_id_map.keys())
    
    img_ids = set()
    for cat_id in cat_ids_to_keep:
        img_ids.update(coco.getImgIds(catIds=cat_id))
    
    images_info = coco.loadImgs(list(img_ids))

    # 3. Get all annotations for these images and categories
    ann_ids = coco.getAnnIds(imgIds=list(img_ids), catIds=cat_ids_to_keep, iscrowd=None)
    annotations_info = coco.loadAnns(ann_ids)

    # 4. Update the category ID for each annotation using our map
    for anno in annotations_info:
        original_id = anno['category_id']
        anno['category_id'] = original_to_new_cat_id_map[original_id]

    # 5. Assemble the final, correctly ordered annotation dictionary
    filter_annos = {
        "images": images_info,
...
def prepare_for_training(path_to_anno_mixedDatasets, anno, existed_data=None, filter_cat_nms=None):
    ...
    if filter_cat_nms:
        print("Filtering the categories which are not in the given category names: ", filter_cat_nms)
